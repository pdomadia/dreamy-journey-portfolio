...
const Work: React.FC = () => {
  const workExperience = [
    {
      ...
      icon: <Laptop size={20} className="text-terracotta" />,
    },
    {
      period: '2019 - 2020',
  ...
  return (
    <section id="work" className="py-20 md:py-28 bg-white/50 relative">
      {/* Background decorative element */}
      <div className="absolute bottom-0 right-0 w-72 h-72 bg-sage/10 rounded-full blur-3xl -z-10" aria-hidden="true"></div>
      ...
      <div className="container-custom">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-0 md:left-1/2 top-0 bottom-0 w-px bg-sand/60 transform md:translate-x-px" aria-hidden="true"></div>
          ...
          <div className="space-y-12 relative">
            {workExperience.map((item, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className="absolute left-0 md:left-1/2 w-12 h-12 bg-cream rounded-full border-4 border-sand/60 transform md:-translate-x-1/2 flex items-center justify-center z-10">
                  {item.icon}
                </div>
                
                {/* Content card */}
                <div 
                  className={`md:w-5/12 bg-white rounded-xl p-6 shadow-sm border border-sand/20 mt-8 md:mt-0 ${
                ...
                >
                  ...
                  <p className="text-sm text-terracotta mb-3">{item.company}</p>
